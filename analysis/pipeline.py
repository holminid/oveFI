    ...
    * flattens every respondent into a 44+ column matrix row aligning scenarios,
    ...
    def run(self, args: Optional[Any] = None) -> List[Result]:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                result_id: Optional[int] = int(idx)  # type: ignore[arg-type]
            except (TypeError, ValueError):
                fallback_id = row.get("respondent_id")
                try:
                    result_id = int(fallback_id) if fallback_id is not None else None
                except (TypeError, ValueError):
                    result_id = None

            res = Result(id=result_id, text=text, features=feats, score=score)
